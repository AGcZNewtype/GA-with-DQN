...
def init(name):
    ...
    # DQN Network
    # input_size, output_size
    # policy_net = DQNAgent(input_size, output_size).to(device)
    policy_net = CCAgent(input_size, output_size).to(device)
    target_net = CCAgent(input_size, output_size).to(device)
    # target_net = DQNAgent(input_size, output_size).to(device)
...
def rollout(name, model, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        if i_episode == 0:
            ...
            torch.save(best_model.state_dict(), 'model\\' + filename + env_name + '_control_DQN.pth')
